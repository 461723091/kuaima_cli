...
func usage(w io.Writer) {
	...
	fmt.Fprintln(w, "  -image-size SIZE        图片尺寸：auto、1024x1024、1536x1024、1024x1536，或 gpt-image-2 自定义 WxH")
	fmt.Fprintln(w, "  -image-quality QUALITY  图片质量：auto、low、medium、high")
	fmt.Fprintln(w, "  -image-count N          image 子命令生成数量")
	fmt.Fprintln(w, "  -image-output-format F  输出格式：png、jpeg、webp")
	fmt.Fprintln(w, "  -image-output-compression N  jpeg/webp 输出压缩，0-100")
	fmt.Fprintln(w, "  -image-background BG    背景：auto、transparent、opaque")
	fmt.Fprintln(w, "  -image-moderation MODE  审核强度：auto 或 low")
	fmt.Fprintln(w, "  -image-action ACTION    Responses 图片工具动作：auto、generate、edit")
	fmt.Fprintln(w, "  -image-mask PATH_OR_URL Images Edit 蒙版图片，仅 image -file ... 时使用")
